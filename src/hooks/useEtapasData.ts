...
export function useEtapasData(boardId: number | null) {
    ...
    useEffect(() => {
        ...
        function fetchPage() {
            const query = `query { boards(ids: ${boardId}) { items_page(limit: 500${cursor ? `, after: "${cursor}"` : ""}) { items { id name column_values { id text ... on MirrorValue { id display_value } ... on FormulaValue { id value display_value } ... on BoardRelationValue { linked_item_ids display_value } } } cursor } } } }`;
